...
function* submitFormSubmittedSaga() {
    try {
        ...
        const isImageValid = yield* validateImage(selectedImage);

        if (user.isLoggedIn && isValidForm && isImageValid) {
            const loginResult = yield call(UserService.login, user.username, user.password); // todo only login again if token expired

            const compressedImage = yield ImageManipulator.manipulateAsync(
                selectedImage.uri,
                [],
